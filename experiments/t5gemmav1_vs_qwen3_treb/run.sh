#!/usr/bin/env bash
# Pod entrypoint for this experiment's TReB eval runs.
# Mirrors the sibling experiments/t5gemma_vs_qwen_treb/run.sh.
#
# Env vars:
#   CONFIG          — path to YAML config (e.g. configs/qwen3_4b_instruct.yaml)
...
#   RUN_ALL         — default 1 (use run_all.sh); 0 = run single CONFIG.
set -euo pipefail
...
if [[ -n "${CODE_REPO:-}" ]]; then
  ...
  git pull --ff-only
  CODE_DIR="$CODE_DIR/experiments/t5gemmav1_vs_qwen3_treb"
fi
cd "$CODE_DIR"
echo "[run.sh] code dir: $(pwd)  ($(git rev-parse --short HEAD 2>/dev/null || echo 'not a git checkout'))"

# --- Eval -----------------------------------------------------------------
# Default: orchestrate variants via run_all.sh.
# Override with RUN_ALL=0 + CONFIG to run a single variant.
RUN_ALL="${RUN_ALL:-1}"
if [[ "$RUN_ALL" == "1" ]]; then
  echo "[run.sh] running run_all.sh"
  set +e
  bash run_all.sh
  EVAL_EXIT=$?
  set -e
else
  CONFIG="${CONFIG:-configs/qwen3_4b_instruct.yaml}"
